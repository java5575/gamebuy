...
public class ImageCheckServlet extends HttpServlet {
    ...
    private int len = 6;
    private int width = 10 * 2 + 12 * len, height = 20;
    ...
    private BufferedImage generateImage(String rand, int width, int height) {
        ...
        g.setFont(new Font("Terminal", Font.PLAIN, 18));//設定字體
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (s == null) {//表示不需要更新驗證碼
            rand = (String) session.getAttribute(getServletName());//用getServletName是因為一個servlet類別建起來兩個物件
        } else {
